...
void myDES2::InitializeKey(char* srcBytes,unsigned int keyN)
{
...
void myDES2::CreateSubKey(char* sz_56key,unsigned int keyN)
{
	char szTmpL[28] = {0};
	char szTmpR[28] = {0};
	char szCi[28] = {0};
...
void myDES2::EncryptData(char* _srcBytes,unsigned int keyN)
{
	char szSrcBits[64] = {0};
	char sz_IP[64] = {0};
	char sz_Li[32] = {0};
...
void myDES2::DecryptData(char* _srcBytes,unsigned int keyN)
{
	char szSrcBits[64] = {0};
	char sz_IP[64] = {0};
	char sz_Li[32] = {0};
...
void myDES2::FunctionF(char* sz_Li,char* sz_Ri,unsigned int iKey,unsigned int keyN)
{
	char sz_48R[48] = {0};
	char sz_xor48[48] = {0};
	char sz_P32[32] = {0};
...
void myDES2::InitialPermuteData(char* _src,char* _dst)
{
	//IP
	for(int i=0;i<64;i++)
	{
...
void myDES2::ExpansionR(char* _src,char* _dst)
{
	for(int i=0;i<48;i++)
	{
...
void myDES2::XOR(char* szParam1,char* szParam2, unsigned int uiParamLength, char* szReturnValueBuffer)
{
...
void myDES2::CompressFuncS(char* _src48, char* _dst32)
{
	char bTemp[8][6]={0};
	char dstBits[4]={0};
	for(int i=0;i<8;i++)
...
void myDES2::PermutationP(char* _src,char* _dst)
{
	for(int i=0;i<32;i++)
	{
...
void myDES2::Bytes2Bits(char *srcBytes, char* dstBits, unsigned int sizeBits)
{
	for(unsigned int i=0; i < sizeBits; i++)
		dstBits[i] = ((srcBytes[i>>3]<<(i&7)) & 128)>>7;
}

void myDES2::Bits2Bytes(char *dstBytes, char* srcBits, unsigned int sizeBits)
{
...
void myDES2::Int2Bits(unsigned int _src, char* dstBits)
{
	for(unsigned int i=0; i < 4; i++)
		dstBits[i] = ((_src<<i) & 8)>>3;
}

void myDES2::Bits2Hex(char *dstHex, char* srcBits, unsigned int sizeBits)
{
...
void myDES2::Hex2Bits(char *srcHex, char* dstBits, unsigned int sizeBits)
{
...
char* myDES2::GetCiphertextInBinary()
{
...
char* myDES2::GetCiphertextInHex()
{
...
char* myDES2::GetCiphertextInBytes()
{
	return szCiphertextInBytes;
}

char* myDES2::GetPlaintext()
{
...
char* myDES2::GetCiphertextAnyLength()
{
	return szFCiphertextAnyLength;
}

char* myDES2::GetPlaintextAnyLength()
{
	return szFPlaintextAnyLength;
}

void myDES2::EncryptAnyLength(char* _srcBytes,unsigned int _bytesLength,unsigned int keyN)
{
	if(_bytesLength == 8)
	{
...
void myDES2::DecryptAnyLength(char* _srcBytes,unsigned int _bytesLength, unsigned int keyN)
{
	if(_bytesLength == 8)
	{
