...
// #endif
	...
	RtlZeroMemory(g_szCurFileKey, MAX_KEY_LENGTH) ;
        ...
        i = Psi_AddProcessInfo("System",TRUE);
 
        i = Psi_AddProcessInfo("explorer.exe",TRUE);

        i = Psi_AddProcessInfo("notepad.exe",TRUE);
    //
...
--*/
{
    ...
    try {		
        ...
        try {
           ...
           File_DecryptBuffer(p2pCtx->SwappedBuffer,Data->IoStatus.Information,NULL,&CryptIndex, Data->Iopb->Parameters.Read.ByteOffset.QuadPart);
					//new_buffer=(PUCHAR)p2pCtx->SwappedBuffer;
			...
			do {
					 ...
					 IRP_NOCACHE|IRP_PAGING_IO|IRP_SYNCHRONOUS_PAGING_IO)))
				{
					...
					///{// use file specified key to encrypt file data(this must be history key)
						...
						///}
					///}
          File_EncryptBuffer(newBuf, writeLen,NULL,&CryptIndex,Data->Iopb->Parameters.Write.ByteOffset.QuadPart);
					//new_buffer=(PUCHAR)newBuf;
